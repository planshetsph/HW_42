...
public class Edge {
	...
	public static void test(String url, String email, String password) {
		...
		boolean isLoginExist = Main.isElementPresent(wait, Main.byLoginForEdge);
		Dimension loginDimension = null;
		Point loginPoint = null;

		if (isLoginExist) {
			WebElement element = wait.until(ExpectedConditions.presenceOfElementLocated(Main.byLoginForEdge));
			loginDimension = element.getSize();
			loginPoint = element.getLocation();
		} else {
			throw new NoSuchElementException("Element " + Main.byLoginForEdge + " is not Found!");
		}
		...
		wait.until(ExpectedConditions.elementToBeClickable(Main.byLoginForEdge)).click();
		...
		Main.output("Edge", title, isCopyrightExist, copyrightDimension, copyrightPoint, copyright, isEmailExist,
